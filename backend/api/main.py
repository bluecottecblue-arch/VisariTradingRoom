...
app.include_router(team.router, prefix="/api/team", tags=["Team"], dependencies=[Depends(require_authenticated)])
app.include_router(research_lab.router, prefix="/api/research", tags=["Research"], dependencies=[Depends(require_authenticated)])
...
app.include_router(leaderboard.router, prefix="/api/leaderboard", tags=["Leaderboard"], dependencies=[Depends(require_authenticated)])
...
async def health():
    ...
    return {
        ...
        "persistence": {
            ...
            "db_mode": "postgres" if "postgresql+psycopg://" in DATABASE_URL else "sqlite",
