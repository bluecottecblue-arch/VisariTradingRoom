...
def build_bot_result(
    ...
    return {
        ...
        "message": message,
        "mql5_code": mql5_code,
        "python_strategy_code": python_strategy_code,
        ...
        "can_generate_code": status == STATUS_VALID,
        "available_exports": available_exports or ["mql5", "python"],
        "validation": validation,
        "usage": usage,
    }
...
def validate_strategy_intake(intake: dict) -> dict:
    ...
    required_text_fields = [
        ...
        ("edge_hypothesis", "Definisci la tesi di edge", "Es. Il mercato sovraestende i breakout di Londra e ribilancia verso la media entro 3-5 barre."),
        ("market_inefficiency", "Definisci l'inefficienza o il comportamento atteso", "Es. Compressione asiatica + espansione di apertura produce follow-through misurabile."),
        ("favorable_regimes", "Definisci i regimi favorevoli", "Es. Trend H4 allineato, volatilita in espansione, no news high impact imminenti."),
        ("adverse_regimes", "Definisci i regimi ostili", "Es. Range stretto, volumi compressi, sessione illiquida, macro event window."),
        ("falsification_triggers", "Definisci cosa smentisce la tesi", "Es. Tre mesi di test OOS con expectancy negativa e breakdown della stabilita walk-forward."),
    ...
    minimum_detail_fields = [
        ...
        ("edge_hypothesis", "Rendi la tesi di edge più concreta", "La tesi deve spiegare quale inefficienza stai cercando di sfruttare e perché non sia solo storytelling.", "Esempio: il prezzo tende a continuare oltre il massimo della compressione asiatica quando l'ATR M15 supera la media a 20 periodi.", 80),
        ("market_inefficiency", "Definisci meglio il comportamento di mercato che vuoi sfruttare", "Serve una spiegazione operativa dell'edge, non solo il pattern grafico.", "Descrivi il meccanismo: compressione, rilascio di volatilita, mean reversion, carry, squeeze, ecc.", 55),
        ("favorable_regimes", "Definisci meglio i regimi favorevoli", "La strategia deve dichiarare chiaramente quando ha probabilita piu alta di funzionare.", "Trend H4 allineato, volatilita sopra mediana a 20 giorni, sessione Londra/New York attiva.", 35),
        ("adverse_regimes", "Definisci meglio i regimi ostili", "Serve sapere quando NON usare il modello per evitare rumore o overfitting narrativo.", "Range stretto, ATR sotto 20th percentile, minuti prima di CPI/NFP, spread anomalo.", 35),
        ("falsification_triggers", "Definisci meglio come invalidare l'idea", "Un modello istituzionale richiede criteri espliciti per dire che la tesi non regge piu.", "Due finestre walk-forward negative consecutive o degradazione OOS oltre il 35%.", 45),
